...
--[[
if not arg[1] then
print("lua swiss_next.lua round")
os.exit()
end
--]]
...
--add round_data to round_datas { h=,l=,table=,section=,NS=,EW=,round=}
for i,v in ipairs(desk_info) do
  local no1 = v.v1.no
  local no2 = v.v2.no
  local a1 = {}
  a1.l = 1
  a1.h = round_datas.boards
  a1.table = i
	a1.section = 1
  a1.NS = no1
  a1.EW = no2
  a1.round = next_round
  table.insert(round_datas,a1)
  local a2 = {}
  a2.l = 1
  a2.h = round_datas.boards
  a2.table = i
  a2.section = 2
  a2.NS = no2
  a2.EW = no1
  a2.round = next_round
  table.insert(round_datas,a2)
end
...
comma.save_file("round_datas.lua",round_datas,"round_datas")
end
